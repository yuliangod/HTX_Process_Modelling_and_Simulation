...
class EmergencyDepartment:
    ...
    def run(self):
        ...
        self.env.run(until=1000)  # Run simulation for n time units
...
if __name__ == "__main__":
    ...
    main_labs = [
            Station(env=env, num_staff=1, name="Main Lab 1", treatment_time_dist=TREATMENT_TIME_DIST, prob_station_needed=1),
            Station(env=env, num_staff=1, name="Main Lab 2", treatment_time_dist=TREATMENT_TIME_DIST, prob_station_needed=0.5),
            Station(env=env, num_staff=1, name="Main Lab 3", treatment_time_dist=TREATMENT_TIME_DIST, prob_station_needed=0.5),
            Station(env=env, num_staff=1, name="Main Lab 4", treatment_time_dist=TREATMENT_TIME_DIST, prob_station_needed=0.5),
        ]
    main_dr_room = Station(env=env, num_staff=1, name="Main Doctor's Room", treatment_time_dist=TREATMENT_TIME_DIST, prob_station_needed=1)
    main_bed = Station(env=env, num_staff=30, name="Main Beds", treatment_time_dist=lambda:random.expovariate(1 / 720), prob_station_needed=0.01)
    
    # define fast track settings
    ft_labs = [
            Station(env=env, num_staff=1, name="FT Lab 1", treatment_time_dist=TREATMENT_TIME_DIST, prob_station_needed=1),
        ]
    ft_dr_room = Station(env=env, num_staff=1, name="FT Doctor's Room", treatment_time_dist=TREATMENT_TIME_DIST, prob_station_needed=1)
